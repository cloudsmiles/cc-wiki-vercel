...
1. 字符串（string）：最基本的类型，可以包含任何数据，如文本、图片等。
2. 哈希（hash）：类似于关联数组或者字典，包含键值对的无序散列表。
3. 列表（list）：基于链表实现的数据结构，可以通过头部或者尾部进行元素的压入和弹出。
4. 集合（set）：包含不重复元素的无序集合，支持交、并、差等操作。
5. 有序集合（sorted set）：和集合类似，但每个元素关联了一个浮点数，可以通过该浮点数进行排序。

以上是Redis支持的五种数据结构。使用这些数据结构可以轻松地实现多种应用场景，如缓存、分布式锁等。
...
![Untitled](RPG从零开始/Database/Redis/数据结构/基础/Untitled.png)
...
![Untitled](RPG从零开始/Database/Redis/数据结构/基础/Untitled%201.png)
