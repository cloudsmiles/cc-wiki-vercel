...
![Untitled](RPG从零开始/Program%20Language/Go/高级/并发编程/上下文Context/Untitled.png)

**Context 与 Goroutine 树**

Context最基本的用法是用来同步信号，多个 Goroutine 同时订阅 `ctx.Done()`管道中的消息，一旦接收到取消信号就立刻停止当前正在执行的工作。
...
这两个私有变量都是通过 `new(emptyCtx)`语句初始化的，它们是指向私有结构体 `[context.emptyCtx](https://draveness.me/golang/tree/context.emptyCtx)`的指针，这是最简单、最常用的上下文类型，就是实现了Context的所有方法，但是没有任何功能
...
![Untitled](RPG从零开始/Program%20Language/Go/高级/并发编程/上下文Context/Untitled%201.png)
...
context.CancelCtx.cancel，该方法会关闭上下文中的 Channel 并向所有的子上下文同步取消信号：
...
context.WithTimeout，context.WithDeadline函数：
...
与context.WithCancel类似，先创建私有结构体，然后与父context绑定关系，并通过 `[time.AfterFunc](https://draveness.me/golang/tree/time.AfterFunc)`创建定时器，当时间超过了截止日期后会调用 `[context.timerCtx.cancel](https://draveness.me/golang/tree/context.timerCtx.cancel)`同步取消信号。
